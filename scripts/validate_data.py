...
def validate_iris_data():
    ...
    try:
        ...
        if not actual_species.issubset(valid_species):
            print(f"❌ Iris dataset contains invalid species: {actual_species - valid_species}")
            return False
...
def validate_root_cause_data():
    ...
    try:
        ...
        if not actual_causes.issubset(valid_causes):
            print(f"❌ Root cause dataset contains invalid causes: {actual_causes - valid_causes}")
            return False
        
        print(f"✅ Root cause analysis dataset validated: {len(df)} rows, {len(df.columns)} columns")
        return True
