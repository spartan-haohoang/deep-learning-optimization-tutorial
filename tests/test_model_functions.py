...
class TestModelFunctions:
    ...
    def test_data_preprocessing(self):
        ...
        if iris_path.exists():
            ...
            assert len(encoded_species) == len(df), "Encoded species should have same length as original"
            assert len(np.unique(encoded_species)) == 3, "Should have 3 unique encoded values"
            ...
            assert scaled_features.shape == features.shape, "Scaled features should have same shape"
            assert np.allclose(scaled_features.mean(axis=0), 0, atol=1e-10), "Scaled features should have zero mean"
            assert np.allclose(scaled_features.std(axis=0), 1, atol=1e-10), "Scaled features should have unit variance"
    
    def test_model_configuration(self):
        """Test model configuration structure."""
        # This would test the base_model_config function if extracted to a module
        expected_keys = [
            "HIDDEN_NODES", "HIDDEN_ACTIVATION", "OUTPUT_NODES", "OUTPUT_ACTIVATION",
            "WEIGHTS_INITIALIZER", "BIAS_INITIALIZER", "NORMALIZATION", "OPTIMIZER",
            "LEARNING_RATE", "REGULARIZER", "DROPOUT_RATE", "EPOCHS", "BATCH_SIZE",
            "VALIDATION_SPLIT", "VERBOSE", "LOSS_FUNCTION", "METRICS"
        ]
        ...
        assert mock_config["OUTPUT_NODES"] == 3, "Output nodes should be 3 for Iris classification"
        assert mock_config["LEARNING_RATE"] > 0, "Learning rate should be positive"
        assert 0 <= mock_config["VALIDATION_SPLIT"] <= 1, "Validation split should be between 0 and 1"
    ...
    def test_optimizer_creation(self):
        ...
        for optimizer_name in optimizers:
            ...
            assert optimizer_name in optimizers, f"Optimizer {optimizer_name} should be supported"
    ...
    def test_data_shapes(self):
        ...
        if iris_path.exists():
            ...
            target = df['Species']
            assert len(target) == features.shape[0], "Target should have same length as features"
